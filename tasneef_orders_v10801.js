...
(function(){
  ...
  const BUILD='V10865-ORDER-PROJECT-MOBILE-STABLE';
  const PAGE_SIZE=50;
  ...
  function mergeProjectRows(){
    // المصدر الأول هو الأعلى أولوية. هذا مهم للإدارة لأن جدول projects الرئيسي
    // يجب أن يبقى المرجع النهائي ولا تستبدله نتيجة RPC أقدم بعد فتح القسم.
    const map=new Map();
    for(const list of arguments){
      normalizeProjectRows(list).forEach(p=>{
        const key=S(p.id);
        if(!map.has(key)) map.set(key,p);
        else{
          const cur=map.get(key);
          map.set(key,{...p,...cur,name:S(cur.name)||S(p.name),code:S(cur.code)||S(p.code)});
        }
      });
    }
  ...
  async function fetchMasterProjects(){
    // select('*') مقصود هنا: بعض نسخ قاعدة البيانات لا تحتوي أعمدة مثل
    // official_name/display_name. طلب عمود غير موجود يفشل الاستعلام كله ويخفي المشاريع.
    const out=[]; let from=0; const size=1000;
    for(let guard=0;guard<50;guard++){
      const {data,error}=await sb().from('projects').select('*').range(from,from+size-1).order('id',{ascending:true});
  ...
  function projectNameById(projectId){
    const id=S(projectId); if(!id)return '';
    const hit=projects().find(p=>S(p.id)===id);
    return S(hit?.name);
  }
  function enrichOrderProject(row){
    if(!row||typeof row!=='object')return row;
    const name=S(row.project_name)||projectNameById(row.project_id);
    return name===S(row.project_name)?row:{...row,project_name:name};
  }
  ...
  function injectStyle(){
    ...
    @media(max-width:1100px){.ov8-layout{grid-template-columns:1fr}.ov8-form{position:static}.ov8-filters{grid-template-columns:repeat(2,1fr)}}@media(max-width:680px){.ov8-kpis,.ov8-grid,.ov8-filters,.ov8-detail-grid,.ov8-finance-row{grid-template-columns:1fr}.ov8-cards{grid-template-columns:minmax(0,1fr)}.ov8-card{min-width:0;padding:11px}.ov8-card-head{align-items:flex-start}.ov8-meta{grid-template-columns:1fr 1fr}.ov8-actions,.ov8-card-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));width:100%}.ov8-actions button,.ov8-card-actions button{width:100%;min-width:0}.ov8-head{align-items:stretch}.ov8-pager{flex-direction:column;align-items:stretch}.ov8-modal{padding:0;align-items:flex-end}.ov8-modal-card{width:100%;max-height:94vh;border-radius:20px 20px 0 0;padding:14px}.ov8-receipt-preview{padding:0}.ov8-receipt-preview-card{width:100%;height:94vh;border-radius:20px 20px 0 0}}@media(max-width:420px){.ov8-meta,.ov8-actions,.ov8-card-actions{grid-template-columns:1fr}.ov8-kpi b{font-size:22px}}
  ...
  function validatePayload(p){const missing=[];if(!p.project_id||!p.project_name||p.project_name==='اختر المشروع')missing.push('المشروع');if(!p.customer_name)missing.push('اسم العميل');if(!p.customer_phone)missing.push('رقم العميل');if(!p.unit_number)missing.push('الوحدة / الموقع');if(!p.details)missing.push('التفاصيل');if(missing.length)throw new Error('أكمل الحقول المطلوبة: '+missing.join('، '));}
  ...
  async function loadFilters(){
    ...
    try{
      ...
      const projectNames=[...new Set([...(Array.isArray(o.projects)?o.projects:[]),...projects().map(p=>p.name)].map(S).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ar'));
