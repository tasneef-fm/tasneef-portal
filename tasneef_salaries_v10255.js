(function(){
  ...
  function buildRows(){
    ...
    const selectedSupervisor = sid ? supervisorName(sid) : '';
    const selectedProject = pid ? (state.projects.find(p=>String(p.id)===String(pid))?.name||'') : '';

    function profileWorkMatchSupervisor(p){
      if(!sid) return true;
      const supKey=normName(selectedSupervisor);
      const empKey=normName(p.employee_name);
      const workKey=normName(p.work_location);
      if(!supKey) return true;
      return empKey===supKey || workKey===supKey || workKey.includes(supKey) || supKey.includes(workKey);
    }
    function profileWorkMatchProject(p){
      if(!pid) return true;
      const prKey=normName(selectedProject);
      const workKey=normName(p.work_location);
      if(!prKey) return true;
      return workKey===prKey || workKey.includes(prKey) || prKey.includes(workKey);
    }

    let rows=(state.profiles||[])
      .filter(p=>p && (p.employee_name || p.residency_name || p.employee_ts_id))
      .filter(p=> type==='all' || type===jobCategory(p.job_title))
      .filter(profileWorkMatchSupervisor)
      .filter(profileWorkMatchProject)
      .sort((a,b)=>num(a.row_order||9999)-num(b.row_order||9999));

    rows=rows.map((p,idx)=>{
      const entityId = p.employee_ts_id || p.id || ('profile_'+idx);
      const sv=savedFor('profile',entityId,month);
      const att=relatedProfileAttendance(p,month);
      const job=p.job_title||'عامل';
      const cat=jobCategory(job);
      let r={
        entity_type:'profile',
        entity_id:entityId,
        profile_id:p.id||null,
        row_order:num(p.row_order||idx+1),
        ...
        employee_name:sv.employee_name || p.employee_name || p.residency_name || '',
        work_location:sv.work_location || p.work_location || (cat==='supervisors'?'FM':''),
        project_name:sv.project_name || p.project_name || p.work_location || '',
        supervisor_id:null,
        supervisor_name:(cat==='supervisors') ? (p.employee_name||'') : (p.work_location||''),
        job_title:sv.job_title || p.job_title || 'عامل',
        start_date:sv.start_date || p.default_start_date || monthStart(month),
        end_date:sv.end_date || p.default_end_date || dateRangeEnd(month),
        work_days:sv.work_days ?? p.default_work_days ?? att.present ?? daysInMonth(month),
        absent_days:sv.absent_days ?? p.default_absent_days ?? att.absent ?? 0,
        payable_days:sv.payable_days ?? p.default_payable_days ?? Math.max(0,(p.default_work_days??dim)-(p.default_absent_days??0)),
        basic_salary:sv.basic_salary ?? p.basic_salary ?? (cat==='supervisors'?2000:(cat==='guards'?1200:1300)),
        allowance:sv.allowance ?? p.allowance ?? (cat==='supervisors'?300:200),
        commission:sv.commission ?? p.default_commission ?? 0,
        deductions:sv.deductions ?? p.default_deductions ?? 0,
        ...
        _allDates:att.dates || [],
        _absentDates:att.absentDates || [],
        _manual_deductions:true
      };
      // إذا المستخدم لم يحفظ تعديل سابقًا، نطابق ملف الإكسل أولًا كما هو.
      if(!sv.employee_name && !sv.employee_ts_id){
        r.gross_salary = p.gross_salary ?? (num(r.basic_salary)+num(r.allowance));
        r.salary_by_days = p.default_salary_by_days ?? ((num(r.gross_salary)/dim)*num(r.work_days));
        r.net_salary = p.default_net_salary ?? (num(r.salary_by_days)+num(r.commission)-num(r.deductions)-num(r.advance_deduction)+num(r.rounding));
        r.payable_days = p.default_payable_days ?? Math.max(0,num(r.work_days)-num(r.absent_days));
        return r;
      }
      return calcRow(r,dim);
    });

    if(q) rows=rows.filter(r=>[r.employee_ts_id,r.residency_name,r.iqama_no,r.employee_name,r.work_location,r.supervisor_name,r.project_name,r.job_title].join(' ').includes(q));
    state.rows=rows;
    renderSalary();
  }
  ...
  async function loadSalary(){
    try{
      ...
      state={workers,projects,users,attendance,settings,saved,profiles,rows:[]}; fillSalaryFilters(); buildRows(); msg('تم تحميل الرواتب - بيانات الإقامة: '+(profiles||[]).length+' موظف');
  ...
  function groupedSalaryRows(rows){
    rows=[...(rows||[])].sort((a,b)=>num(a.row_order||9999)-num(b.row_order||9999));
    const supervisors=rows.filter(r=>jobCategory(r.job_title)==='supervisors');
    const others=rows.filter(r=>jobCategory(r.job_title)!=='supervisors');
    if(!supervisors.length) return rows.slice().sort((a,b)=>(a.supervisor_name||'').localeCompare(b.supervisor_name||'', 'ar') || (a.employee_name||'').localeCompare(b.employee_name||'', 'ar'));
    ...
    supervisors.forEach(s=>{
      ...
      others.filter(r=>{
        ...
        const byName=(s.employee_name||s.supervisor_name) && r.supervisor_name && String(r.supervisor_name).trim()===String(s.employee_name||s.supervisor_name).trim();
        return byId || byName;
      }).sort((a,b)=>(a.employee_name||'').localeCompare(b.employee_name||'', 'ar')).forEach(r=>{ used.add(r); out.push(r); });
    });
    others.filter(r=>!used.has(r)).sort((a,b)=>(a.supervisor_name||'').localeCompare(b.supervisor_name||'', 'ar') || (a.employee_name||'').localeCompare(b.employee_name||'', 'ar')).forEach(r=>out.push(r));
    return out;
  }
  ...
  function inject(){
    ...
    const main=document.querySelector('main.content'); const sec=document.createElement('section'); sec.id='salaries'; sec.className='page hidden'; sec.innerHTML=`
      ...
      <div class="card"><div class="table-head"><h2>الرواتب</h2><span class="badge green">${VERSION}</span></div><div class="salary-note">تمت مطابقة بيانات الرواتب مع ملف Excel المرفق: المشرف وتحت عماله، مكان العمل من الملف، أيدي الموظف TS، اسم الإقامة ورقم الإقامة.</div><div id="salaryMsg" class="msg hidden"></div>
