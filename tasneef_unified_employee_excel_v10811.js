...
(function(){
  ...
  const VERSION='V10836-REFERENCE-FORMULA';
  ...
  function excelDateValue(v){const x=dateOnly(v);if(!/^\d{4}-\d{2}-\d{2}$/.test(x))return null;const d=new Date(x+'T12:00:00');return Number.isNaN(d.getTime())?null:d;}
  ...
  async function addMainPayrollSheet(wb,month,details,filters){
    ...
    const computed=[];
    details.forEach((x,i)=>{
      const eligible=Math.max(0,Math.min(PAYROLL_DAYS,N(x.eligibleDays)||dateDaysInclusive(x.eligibilityStart,x.eligibilityEnd)));
      const due=Math.max(0,eligible-N(x.absent)-N(x.unpaidLeave));
      const gross=N(x.basic)+N(x.allowances);
      const periodSalary=gross/PAYROLL_DAYS*due;
      const beforeRounding=periodSalary+N(x.commissions)-N(x.deductions)-N(x.advances);
      const rounding=Math.round(beforeRounding)-beforeRounding;
      const net=beforeRounding+rounding;
      computed.push({eligible,due,gross,periodSalary,rounding,net});
      const row=ws.addRow([i+1,x.code,x.iqamaName,x.name,x.iqamaNumber,x.roleLabel,x.originalJobTitle,x.supervisorName,x.workplace,excelDateValue(x.workStart),excelDateValue(x.eligibilityStart),excelDateValue(x.eligibilityEnd),eligible,0,0,0,0,0,x.basic,x.allowances,0,0,0,x.commissions,x.deductions,x.advances,0,0,x.notes,x.status,x.employeeKey]);
      const rn=row.number;
      row.getCell(14).value={formula:`IFERROR(XLOOKUP($AE${rn},'التحضير الشهري'!$J:$J,'التحضير الشهري'!$AO:$AO,0),0)`,result:x.attendanceCount};
      row.getCell(15).value={formula:`IFERROR(XLOOKUP($AE${rn},'التحضير الشهري'!$J:$J,'التحضير الشهري'!$AP:$AP,0),0)`,result:x.absent};
      row.getCell(16).value={formula:`IFERROR(XLOOKUP($AE${rn},'التحضير الشهري'!$J:$J,'التحضير الشهري'!$AQ:$AQ,0),0)`,result:x.paidLeave};
      row.getCell(17).value={formula:`IFERROR(XLOOKUP($AE${rn},'التحضير الشهري'!$J:$J,'التحضير الشهري'!$AR:$AR,0),0)`,result:x.unpaidLeave};
      /* معادلة ملف المحاسب: أيام الفترة من البداية والنهاية، مع خصم الغياب والإجازة غير المدفوعة. */
      row.getCell(18).value={formula:`MAX(0,MIN(M${rn},IF(OR(K${rn}="",L${rn}=""),M${rn},L${rn}-K${rn}+1))-O${rn}-Q${rn})`,result:due};
      /* نفس معادلات الملف المرجعي: الإجمالي = الأساسي + البدلات. */
      row.getCell(21).value={formula:`S${rn}+T${rn}`,result:gross};
      row.getCell(22).value={formula:`IFERROR(U${rn}/30,0)`,result:gross/30};
      /* راتب الأيام = الإجمالي / 30 × الأيام المستحقة. */
      row.getCell(23).value={formula:`IFERROR(U${rn}/30*R${rn},0)`,result:periodSalary};
      row.getCell(27).value={formula:`ROUND(W${rn}+X${rn}-Y${rn}-Z${rn},0)-(W${rn}+X${rn}-Y${rn}-Z${rn})`,result:rounding};
      /* الصافي = راتب الأيام + العمولات - الخصومات - السلف + جبر الكسور. */
      row.getCell(28).value={formula:`W${rn}+X${rn}-Y${rn}-Z${rn}+AA${rn}`,result:net};
      row.height=24;const sup=x.roleGroup==='supervisor';
      row.eachCell({includeEmpty:true},(c,col)=>styleCell(c,{fill:sup?'DCEFE8':(i%2?'F8FBFA':'EEF7F4'),bold:sup,numFmt:moneyCols.has(col)?'#,##0.00 "ر.س"':([1,13,14,15,16,17,18].includes(col)?'0':undefined)}));
      [10,11,12].forEach(col=>{row.getCell(col).numFmt='yyyy-mm-dd';});
      [20,24,25,26,29].forEach(col=>{row.getCell(col).protection={locked:false};row.getCell(col).fill={type:'pattern',pattern:'solid',fgColor:{argb:ARGB('FFF7D6')}};});
    });
    const first=4,last=3+details.length,total=ws.addRow(['الإجمالي','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','']);
    ws.mergeCells(total.number,1,total.number,18);
    for(let col=19;col<=28;col++){
      const letter=ws.getColumn(col).letter;
      total.getCell(col).value={formula:`SUM(${letter}${first}:${letter}${last})`,result:computed.reduce((a,c,i)=>a+N(({19:details[i]?.basic,20:details[i]?.allowances,21:c.gross,22:c.gross/30,23:c.periodSalary,24:details[i]?.commissions,25:details[i]?.deductions,26:details[i]?.advances,27:c.rounding,28:c.net})[col]),0)};
    }
    total.eachCell({includeEmpty:true},(c,col)=>styleCell(c,{fill:'D7EEE6',bold:true,numFmt:moneyCols.has(col)?'#,##0.00 "ر.س"':undefined}));
    ws.columns=[6,14,28,23,16,12,20,22,30,14,14,14,14,12,11,16,17,14,15,13,15,13,18,13,13,13,13,15,30,11,18].map(width=>({width}));
    ws.getColumn('AE').hidden=true;ws.autoFilter={from:'A3',to:`AD${Math.max(3,last)}`};ws.headerFooter.oddHeader='شركة تصنيف لإدارة المرافق — كشف الرواتب';ws.headerFooter.oddFooter='صفحة &P من &N';
    /* V10836: الورقة قابلة للتعديل، والمعادلات مطابقة لصيغة ملف المحاسب المرفق. */ return ws;
  }
