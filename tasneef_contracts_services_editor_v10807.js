...
(function(){
  ...
  function contractsViewActive(){const el=$('contracts');return !!(state.open||(el&&!el.classList.contains('hidden')));}
  function boot(){ensureModal();ensureActionLayer();installContractsScope();setTimeout(installContractsScope,900);setTimeout(installContractsScope,2200);window.addEventListener('tasneef:project-updated',e=>{if(!contractsViewActive())return;const pid=S(e.detail?.projectId);loadAllProjects(true).then(()=>{renderContractsOnly();renderContractAlerts(true);if(state.open&&S(state.projectId)===pid&&!state.dirty)openEditor(pid,state.mode,true);});});console.log('Tasneef ContractsServicesEditor '+VERSION+' loaded — lazy section data');}
