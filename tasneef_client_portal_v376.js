...
(function(){
  ...
  function projectSlug(){
    const pn=(projectName(getFilters().projectId)||'tasneef').toString().trim();
    const cleaned=pn.replace(/[^a-zA-Z0-9\u0600-\u06FF]+/g,'-').replace(/^-+|-+$/g,'').slice(0,24);
    return cleaned || 'tasneef';
  }
  function shortCacheKey(longUrl){ return 'tasneef_short_url_v376_'+simpleHash(longUrl); }
  async function shortenUrlV376(longUrl, force){
    ...
    const alias='tasneef-'+projectSlug().replace(/[\u0600-\u06FF]/g,'').replace(/[^a-zA-Z0-9-]/g,'').toLowerCase().slice(0,18)+'-'+simpleHash(longUrl).slice(0,5);
    const services=[
  ...
  function renderLinkBoxV376(longUrl, shortUrl, loading){
    ...
    box.innerHTML=`<div style="direction:rtl;text-align:right;font-weight:800;color:#073e31;margin-bottom:6px">بوابة شركة تصنيف لإدارة المرافق</div>
      <div style="direction:ltr;text-align:left;word-break:break-all"><small style="color:#60706a">الرابط المختصر:</small><br><b>${esc(loading?'جاري إنشاء الرابط المختصر...':(shortUrl||'لم يتم إنشاؤه بعد'))}</b></div>
