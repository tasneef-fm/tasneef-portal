(function(){
  ...
  const VERSION='V450 رواتب مبنية من الصفر';
  ...
  function input(r,k,w=110,type='text'){return `<input class="sal-input" type="${type}" style="width:${w}px" value="${esc(r[k]??'')}" onchange="tasneefSalariesUnifiedV440.update('${esc(r.entity_id)}','${k}',this.value)" oninput="tasneefSalariesUnifiedV440.update('${esc(r.entity_id)}','${k}',this.value)">`;}
  ...
  async function load(){try{if(!window.sb)throw new Error('Supabase غير جاهز'); const month=$('salaryMonth')?.value||today().slice(0,7), start=mStart(month), end=mEnd(month); msg('جاري تحميل الرواتب من التوزيع الموحد فقط...'); const [emp,wrk,dist,att,projects,saved]=await Promise.all([tryAll('employees_master_v386','*'),tryAll('workers','*'),tryAll('monthly_distribution','*'),tryAll('attendance','*',q=>q.gte('attendance_date',start).lte('attendance_date',end)),tryAll('projects','*'),tryAll('monthly_salaries','*',q=>q.eq('salary_month',start))]); st.workers=mergeWorkers(emp,wrk); st.dist=dist; st.attendance=att; st.projects=projects; st.saved=saved; fillFilters(month); buildRows(); msg('تم بناء الرواتب من الصفر: التوزيع ثم بيانات العامل ثم الغياب.');}catch(e){console.error(e); msg('فشل تحميل الرواتب: '+(e.message||e),true);}}
  ...
  window.tasneefSalariesUnifiedV440=api; window.tasneefSalariesV10267=api; window.tasneefSalariesV10281=api;
