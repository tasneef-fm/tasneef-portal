...
(function(){
  ...
  function durationText(t){ const start=Date.parse(dateRaw(t)||''); const closed=S(t.status)==='closed'; const end=closed?Date.parse(t.closed_at||t.closedAt||t.closed_date||t.resolved_at||t.updated_at||dateRaw(t)):Date.now(); if(isNaN(start)||isNaN(end)) return '-'; let mins=Math.max(0,Math.round((end-start)/60000)); const days=Math.floor(mins/1440); mins=mins%1440; const hrs=Math.floor(mins/60), mm=mins%60; const txt=days?`${days} يوم ${hrs} س`:(hrs?`${hrs} س ${mm} د`:`${mm} د`); return closed?`أغلق خلال ${txt}`:`مفتوح منذ ${txt}`; }
  ...
  function exportFilteredCsv(){ const rows=rowsForExport(); const heads=['رقم التكت','التاريخ','المشروع','المشرف','نوع المشكلة','الأولوية','الحالة','مدة إغلاق / فتح التكت','المستلم','المغلق','الوصف','الحل']; const lines=[heads.map(csvEscape).join(',')].concat(rows.map(t=>[ticketNo(t),fmtDate(t),ticketProject(t),ticketSupervisor(t),t.title,priorityLabel(t.priority),statusLabel(t.status),durationText(t),ticketRecipient(t),ticketClosedBy(t),t.description,t.closure_note].map(csvEscape).join(','))); const blob=new Blob(['\ufeff'+lines.join('\n')],{type:'text/csv;charset=utf-8'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='tickets_filtered_'+new Date().toISOString().slice(0,10)+'.csv'; a.click(); setTimeout(()=>URL.revokeObjectURL(a.href),1000); }
  ...
  function exportFilteredPdf(){
    ...
    const html=`<!doctype html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>تقرير التكتات - شركة تصنيف</title><style>
      @page{size:A4 landscape;margin:9mm}*{box-sizing:border-box}body{font-family:Tahoma,Arial,sans-serif;margin:0;background:#fff;color:#133d33;font-size:11px;-webkit-print-color-adjust:exact;print-color-adjust:exact}.sheet{padding:14px;border:2px solid #0b5a49;min-height:100vh}.head{display:flex;align-items:center;justify-content:space-between;border-bottom:3px solid #0b5a49;padding-bottom:10px;margin-bottom:10px}.brand{display:flex;align-items:center;gap:10px}.logoBox{width:72px;height:58px;border:2px solid #d5b15d;border-radius:16px;display:grid;place-items:center;background:#f8fbfa;overflow:hidden}.logoBox img{max-width:68px;max-height:52px}.logoText{font-weight:900;color:#0b5a49;font-size:17px}.brand h1{font-size:18px;margin:0;color:#0b5a49}.brand p{margin:3px 0 0;color:#687b74}.title{text-align:left}.title h2{margin:0;font-size:24px;color:#0b5a49}.title p{margin:4px 0;color:#687b74}.meta{border:1px solid #dce9e4;background:#f3faf7;border-radius:12px;padding:9px;margin:10px 0;line-height:1.8}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:10px 0}.kpi{border:1px solid #dce9e4;border-radius:12px;padding:10px;text-align:center;background:#fff}.kpi b{display:block;color:#0b5a49;font-size:18px;margin-top:3px}table{width:100%;border-collapse:collapse;margin-top:10px;table-layout:fixed}th{background:#0b5a49;color:#fff;font-weight:900;padding:6px 4px;border:1px solid #0b5a49;white-space:normal;font-size:10.5px;line-height:1.25;text-align:center}td{border:1px solid #d9e6e1;padding:5px 4px;vertical-align:top;color:#12382f;font-size:10.5px;line-height:1.35;text-align:center;overflow-wrap:anywhere;word-break:normal}.tickets-report-table col.c-no{width:3.5%}.tickets-report-table col.c-ticket{width:6.5%}.tickets-report-table col.c-date{width:8.5%}.tickets-report-table col.c-project{width:6.5%}.tickets-report-table col.c-supervisor{width:6.5%}.tickets-report-table col.c-type{width:8%}.tickets-report-table col.c-priority{width:5%}.tickets-report-table col.c-status{width:5.5%}.tickets-report-table col.c-duration{width:12%}.tickets-report-table col.c-recipient{width:6%}.tickets-report-table col.c-closer{width:6%}.tickets-report-table col.c-desc{width:26%}.type-cell{font-size:10px;line-height:1.25}.duration-cell{font-size:10px;font-weight:800;color:#0b5a49}.desc{text-align:right;line-height:1.45;font-size:10px}.desc b{color:#0b5a49}.nowrap{white-space:nowrap}.status{font-weight:900}tbody tr:nth-child(even){background:#f7fbfa}.closed{color:#06703b}.open{color:#b00020}.processing{color:#8a5a00}.foot{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:16px}.sign{border:1px solid #d9e6e1;border-radius:12px;min-height:66px;padding:10px}.note{text-align:center;color:#0b5a49;margin-top:10px;font-weight:800}.copy-label{display:inline-block;margin-top:8px;background:#0b5a49;color:#fff;border-radius:999px;padding:7px 18px;font-weight:900}.noPrint{display:none}@media print{.sheet{border-radius:0}.sign,tr{break-inside:avoid}}
    </style></head><body><div class="sheet"><div class="head"><div class="brand"><div class="logoBox"><img src="tasneef_logo_print.png" onerror="this.style.display='none';this.nextElementSibling.style.display='block'"><span class="logoText" style="display:none">تصنيف</span></div><div><h1>شركة تصنيف لإدارة المرافق</h1><p>تقرير تكتات العملاء حسب الفلاتر المحددة</p></div></div><div class="title"><h2>تقرير التكتات</h2><p>تاريخ الإصدار: ${E(generatedAt)}</p></div></div><div class="meta"><b>الفلاتر المطبقة:</b> ${E(filterLabel(f))}</div><div class="kpis"><div class="kpi">إجمالي النتائج<b>${counts.total}</b></div><div class="kpi">مفتوح<b>${counts.open}</b></div><div class="kpi">تحت المعالجة<b>${counts.proc}</b></div><div class="kpi">مغلق<b>${counts.closed}</b></div></div><table class="tickets-report-table"><colgroup><col class="c-no"><col class="c-ticket"><col class="c-date"><col class="c-project"><col class="c-supervisor"><col class="c-type"><col class="c-priority"><col class="c-status"><col class="c-duration"><col class="c-recipient"><col class="c-closer"><col class="c-desc"></colgroup><thead><tr><th>م</th><th>رقم التكت</th><th>التاريخ</th><th>المشروع</th><th>المشرف</th><th>نوع المشكلة</th><th>الأولوية</th><th>الحالة</th><th>مدة الإغلاق / الفتح</th><th>المستلم</th><th>المغلق</th><th>الوصف / الإجراء</th></tr></thead><tbody>${rows.map((t,i)=>`<tr><td class="nowrap">${i+1}</td><td class="nowrap">${E(ticketNo(t))}</td><td>${E(fmtDate(t))}</td><td>${E(ticketProject(t))}</td><td>${E(ticketSupervisor(t))}</td><td class="type-cell">${E(t.title||'-')}</td><td>${E(priorityLabel(t.priority))}</td><td class="status ${S(t.status)==='closed'?'closed':S(t.status)==='processing'?'processing':'open'}">${E(statusLabel(t.status))}</td><td class="duration-cell">${E(durationText(t))}</td><td>${E(ticketRecipient(t))}</td><td>${E(ticketClosedBy(t))}</td><td class="desc"><b>الوصف:</b> ${E(t.description||'-')}${t.closure_note?`<br><b>الإجراء:</b> ${E(t.closure_note)}`:''}</td></tr>`).join('') || '<tr><td colspan="12" style="text-align:center;padding:25px">لا توجد تكتات مطابقة للفلاتر</td></tr>'}</tbody></table><div class="note report-disclaimer">تم إنشاء هذا التقرير من نظام شركة تصنيف لإدارة المرافق، ويعتبر معتمدًا ما لم يبرر العميل خلاف ذلك.</div><div class="copy-label">نسخة الإدارة</div></div><script>window.onload=function(){setTimeout(function(){window.print()},650)}</script></body></html>`;
