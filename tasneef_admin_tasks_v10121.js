/* Tasneef v10121 - Administrative Tasks Supabase Online Only Fix
   Scope: new section only. Does not touch finance, inventory, orders, tickets, contracts, attendance, monthly, or permissions.
*/
(function(){
  'use strict';
  if(window.__tasneefAdminTasksV10121) return;
  window.__tasneefAdminTasksV10121 = true;

  const VERSION='v10121-admin-tasks-advanced';
  const SUPABASE_URL='https://zmjdqiswytxlbfgnfjfv.supabase.co';
  const SUPABASE_ANON_KEY='sb_publishable_ADsAC5MtBCusDgX62c8NaQ_LyyuTPeb';
  const TABLE='admin_tasks';
  const LOCAL_KEY='tasneef_admin_tasks_v10121_local';
  const DISMISS_PREFIX='tasneef_admin_task_dismiss_v10121_';
  ...
  async function loadTasks(){
    try{
      ...
      state.rows=[];
      state.online=false;
      say('تعذر تحميل المهام من Supabase. تأكد من تشغيل SQL ووجود اتصال إنترنت.','err');
    }
    state.loaded=true;
    return state.rows;
  }
  ...
  async function saveTask(payload){
    ...
    try{
      ...
      say('لم يتم حفظ المهمة أونلاين. شغّل ملف SQL في Supabase وتأكد من الاتصال ثم حاول مرة أخرى.','err');
      alert('لم يتم حفظ المهمة أونلاين في Supabase، لذلك لن تظهر للمستلم. شغّل ملف SQL وتأكد من الاتصال ثم أعد الحفظ.');
      return null;
    }
  }
