(function(){
    ...
    if(type==='workers' || type==='all'){
      uniqueWorkerGroups().forEach(g=>{
        ...
        let r={
          ...
          payment_method:'', notes: sv.notes||set.notes||'', _allDates:att.dates, _absentDates:att.absentDates
        };
        // بداية ونهاية الخدمة من الحضور والغياب، ويمكن تعديلها؛ الحساب يعتمد على الفترة المختارة.
        applyPeriodFromDates(r, month);
        const autoDed = (num(r.basic_salary)+num(r.allowance))/dim*num(r.absent_days);
        // V10239: الخصومات تبدأ بخصم الغياب دائمًا. إذا كان محفوظًا خصم أكبر من خصم الغياب نعتبر الفرق خصمًا يدويًا إضافيًا.
    ...
    if(type==='supervisors' || type==='all'){
      state.users.filter(u=>String(u.role||'')==='supervisor' && u.is_active!==false).forEach(u=>{
        ...
        let r={
          ...
          commission: sv.commission??0, deductions: sv.deductions??0, rounding: sv.rounding??0, advance_deduction: sv.advance_deduction??0,
          payment_method:'', notes: sv.notes||set.notes||'', _allDates:span.dates||[], _absentDates:[]
        };
        applyPeriodFromDates(r, month);
        rows.push(calcRow(r,dim));
      });
  ...
  function rowInput(r,k,cls='sal-input money'){ return `<input class="${cls}" data-key="${k}" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r[k]??0)}" onchange="tasneefSalariesV10239.update('${r.entity_type}','${r.entity_id}','${k}',this.value)">`; }
  function dateInput(r,k){ return `<input type="date" class="sal-input" style="width:135px" data-key="${k}" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r[k]||'')}" onchange="tasneefSalariesV10239.update('${r.entity_type}','${r.entity_id}','${k}',this.value)">`; }
  function renderSalary(){
    ...
    body.innerHTML=rows.map((r,i)=>`<tr data-sal-row="${r.entity_type}_${r.entity_id}">
      ...
      <td><input class="sal-input" data-key="notes" data-type="${r.entity_type}" data-id="${r.entity_id}" value="${esc(r.notes||'')}" onchange="tasneefSalariesV10239.update('${r.entity_type}','${r.entity_id}','notes',this.value)"></td>
  ...
  function inject(){
    ...
    const main=document.querySelector('main.content'); const sec=document.createElement('section'); sec.id='salaries'; sec.className='page hidden'; sec.innerHTML=`
      ...
      <div class="card"><div class="table-head"><h2>الرواتب</h2><span class="badge green">${VERSION}</span></div><div class="salary-note">مكان العمل ثابت FM. تاريخ البداية والنهاية من الحضور والغياب وقابل للتعديل. عند تعديل البداية أو النهاية يعاد حساب أيام العمل والغياب والخصومات والصافي فورًا.</div><div id="salaryMsg" class="msg hidden"></div>
      <div class="filters"><div><label>الشهر</label><input type="month" id="salaryMonth" value="${today().slice(0,7)}" onchange="tasneefSalariesV10239.load()"></div><div><label>نوع الكشف</label><select id="salaryType" onchange="tasneefSalariesV10239.buildRows()"><option value="supervisors">رواتب المشرفين</option><option value="workers">رواتب العمال</option><option value="all">الكل</option></select></div><div><label>المشرف</label><select id="salarySupervisor" onchange="tasneefSalariesV10239.buildRows()"><option value="">كل المشرفين</option></select></div><div><label>المشروع</label><select id="salaryProject" onchange="tasneefSalariesV10239.buildRows()"><option value="">كل المشاريع</option></select></div><div><label>بحث</label><input id="salarySearch" oninput="tasneefSalariesV10239.buildRows()" placeholder="اسم الموظف"></div></div>
      <div class="salary-actions"><button onclick="tasneefSalariesV10239.load()">تحديث الرواتب</button><button class="light" onclick="tasneefSalariesV10239.save(false)">حفظ التعديلات</button><button class="light" onclick="tasneefSalariesV10239.save(true)">اعتماد الرواتب</button><button class="light" onclick="tasneefSalariesV10239.print()">طباعة</button><button class="light" onclick="tasneefSalariesV10239.exportExcel()">تصدير Excel</button></div><div id="salaryKpis" class="kpis small"></div>
  ...
  window.tasneefSalariesV10239={inject,load:loadSalary,buildRows,update,save:saveSalary,print:printSalary,exportExcel:exportSalaryExcel};
