...
(function(){
  'use strict';
  if(window.__tasneefAdminTasksV10124) return;
  window.__tasneefAdminTasksV10124 = true;

  const VERSION='v10124-admin-tasks-syntax-sidebar-stable';
  const SUPABASE_URL='https://zmjdqiswytxlbfgnfjfv.supabase.co';
  const SUPABASE_ANON_KEY='sb_publishable_ADsAC5MtBCusDgX62c8NaQ_LyyuTPeb';
  const TABLE='admin_tasks';
  const LOCAL_KEY='tasneef_admin_tasks_v10124_local';
  const DISMISS_PREFIX='tasneef_admin_task_dismiss_v10124_';
  ...
  async function loadTasks(){
    try{
      ...
      state.rows=[];
      state.online=false;
      say('تعذر تحميل المهام من Supabase. شغّل SQL v10124 ثم أعد فتح الصفحة.','err');
    }
    state.loaded=true;
    return state.rows;
  }
  ...
  async function saveTask(payload){
    ...
    try{
      ...
      console.error('v10124 save online failed', e);
      const details=(e && e.message) ? String(e.message).slice(0,700) : '';
      say('لم يتم حفظ المهمة أونلاين. شغّل SQL v10124 وتأكد من ظهور جدول admin_tasks.','err');
      alert('لم يتم حفظ المهمة أونلاين في Supabase، لذلك لن تظهر للمستلم.\n\nالسبب من Supabase:\n' + details + '\n\nالحل: شغّل ملف admin_tasks_v10124.sql كاملًا ثم اعمل Ctrl+F5 وأعد الحفظ.');
      return null;
    }
  }
  ...
  function ensureAdminTasksNav(){
    let btn=document.querySelector('button.nav[data-page="adminTasks"], button[data-page="adminTasks"], button[data-section="adminTasks"], #at10124AdminTasksNav');
    if(!btn){
      btn=document.createElement('button');
      btn.className='nav';
      btn.id='at10124AdminTasksNav';
